...
% DVS data read
if(size(DVStype,1)==0)
fprintf('\n extract %f event information from address \n',size(DVStype,1));
...
% APS data read
if(size(APStype,1)==0)
fprintf('\n extract %f APS information from address \n',size(APStype,1));
...
APSTs   = APSTs(APSctrl_flip==1);
end
% IMU data read

if(size(IMUtype,1)==0)
fprintf('\n extract %f IMU information from address \n',size(IMUtype,1));
...
IMUsample_uint32 = (bitshift(bitand(addr(IMUtype),IMUmask),-IMUshift)); % y addresses

IMUsample_neg = int16(bitget(IMUsample_uint32,16));
IMUsample = int16(bitset(IMUsample_uint32,16,0)) + (-2^15)*IMUsample_neg;
...
Temperature = IMUsample_uint32(IMU_Temp);
...
IMU     = double([Accel_X, Accel_Y, Accel_Z, Gyro_X ,Gyro_Y ,Gyro_Z ,Temperature]); 
% This may occur error due to packet loss.
IMUTs   = IMUStreamTs(IMU_Ax);
end

fprintf('\nextraction takes %f seconds \n', toc);
% yyaxis left
% plot(Ts(APStype));
% yyaxis right
% plot(APSctrl_flip==1);
